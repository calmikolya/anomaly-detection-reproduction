...
- Эпохи: 8 (вместо 700 в статье) — тест на 1 эпохе показал ~49 минут/эпоху на CPU под
  нагрузкой, поэтому пришлось резко урезать эпохи, чтобы уложиться в дедлайн
- Из-за такого урезания pixel-AUROC заметно ниже контроля (0.587 против ~97.5%) —
  сеть-сегментатор просто не успела как следует научиться локализовать дефект за
  8 эпох, хотя на уровне целой картинки (image-AUROC 0.900) результат уже приличный
